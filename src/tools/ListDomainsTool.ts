...
/**
 * Klasse zur Behandlung der 'holaspirit_list_domains' Tool-Anfrage.
 * Diese Klasse kapselt die Logik und den HolaspiritClient.
 */
export class ListDomainsTool implements ToolModule {
    private holaClient: HolaspiritClient;

    /**
     * Metadaten des Tools
     */
    toolName = 'holaspirit_list_domains';
    toolDescription = 'List all domains in the organization';
    inputSchema = schemas.ListDomainsRequestSchema;
    ...
    /**
     * Behandelt die 'holaspirit_list_domains' Tool-Anfrage.
     * Ruft eine paginierte Liste von Domains ab und gibt sie zurück.
     ...
     * @throws {Error} Wenn Domains nicht gefunden werden oder die API-Antwort ungültig ist.
     */
    async execute(request: z.infer<typeof CallToolRequestSchema>) {
        ...
        const parsed = parsePaginatedHolaspiritResponse(
            apiResponse as any,
            schemas.ListDomainsResponseSchema.shape.items,
        );

        const finalParsed = schemas.ListDomainsResponseSchema.parse(parsed);
